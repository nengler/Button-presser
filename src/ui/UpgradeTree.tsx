    ...
    <div className="pix">
      {rows.map(function (row, y) {
        return row.split("").map(function (ch, x) {
          return ch === "X" ? (
            <i key={`${x}-${y}`} style={{ left: x, top: y, background: color }} />
          ) : null;
        });
      })}
    </div>
...
export function UpgradeTree({
  ...
  const segs = TREE_NODES.flatMap(function (n) {
    const childOpen = parentsOwned(n, progress);
    return n.parents.flatMap(function (pid) {
      ...
      return elbow(p.x + half, p.y + half, n.x + half, n.y + half).map(function (box, i) {
        return {
          ...box,
          key: `${pid}-${n.id}-${i}`,
          lit,
  ...
  return (
    <div className="tree">
      ...
      <div className={`tree-inspect${canBuy ? " hot" : ""}`}>
        ...
        <button type="button" className="btn press tree-buy" disabled={!canBuy} onClick={buy}>
